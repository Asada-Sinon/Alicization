...
- **本文档初稿没有运行任何 JAX 实验**——三个设计的 falsifiable prediction 起初
  全部标注 [提案,非结论]。**设计二现已实现并跑过 6 配对种子实测,升级为 [本世界
  实测],见新增的 §6。** 设计一、设计三仍是未验证提案。
...
## 6. [本世界实测] 设计二落地：6 配对种子验证 + 默认开启

设计二（恐惧场折入 `pred` 通道）已按 §3.2 的规格实现并验证。缘起：`docs/scale_and_density.md`
把种群密度定量校验后，用户点明了机制链——**猎物"像蚂蚁窝一样到处都是"，捕食者才能足不出户
坐等猎物流经**，于是要求"捕食者离开河道 + 降占比，捕食者能量机制可调"。设计二正是文档集里排第一
的对症机制，且零 `in_dim` 改动、不作废种群。

**实现**（`state.py`/`step.py`/`sensors.py`/`config.py`，`in_dim` 与 `genome_size` 不变）：
`WorldState` 新增 `fear: [n_cells]` 场；`step.py` 用食肉者（diet>0.5）位置散射累加 + `fear_decay`
衰减（紧邻 `trample` 那段，同一原语）；`sensors.sense` 把前方采样的 `fear`（与 `water_ch`/`slope`
同一段 `cells`）按 `fear_sense_scale` 与瞬时 `pred_val` 取 `max` 折进 `pred` 通道。`fear_rate>0`
是编译期分支：设 0 时沉积与折叠都从 jit trace 消失、`fear` 恒为 0、行为逐位复现旧基线（即消融对照）。

**两轮 6 配对种子实测（seed 0–5，15k 步，`--set` 覆盖）：**

| 臂 | carn_speed（游走） | carn_water_dist（离水） | carn_frac | death_thirst_age | 判读 |
| --- | --- | --- | --- | --- | --- |
| control | 1.5 | ~21 | 12–14% | ~106–112 | 基线 |
| 弱：rate0.01 scale1.0 | +0.16（4/6） | −0.8（2/6） | 噪声（离群种子驱动） | +5.6 | **零结果** |
| 弱：rate0.01 scale2.0 | +0.07 | −0.9（4/6） | −1.0（噪声） | +2.3 | **零结果** |
| **强：rate0.05 scale3.0（选定默认）** | **+0.91（6/6）** | +1.4（3/6，弱） | **−1.9（5/6 下降）** | **+4.9** | **部分阳性** |
| 强：rate0.1 scale4.0 decay0.995 | +0.88（5/6） | +2.1（5/6，弱） | −1.1（5/6 下降） | +1.6 | 部分阳性（不优于上） |

**结论（[本世界实测]）：**
1. **弱设置（rate0.01）零结果**，且单种子探针（seed 0）一度显示 `carn_water_dist` 18→25 的漂亮信号，
   6 种子一摊开完全是噪声（种子间 −6.8～+5.4）——**又一次印证 `conventions.md` 的铁律：绝不在单种子上
   调这套生态。** 弱设置沉积太弱，恐惧场累积不起来。
2. **强设置（rate0.05/scale3.0）是可落地的部分阳性**：**捕食者从"坐等"转向"游走"是最硬的信号
   （carn_speed 1.5→2.4，6/6 种子，配对 p=0.031）**，直接回应用户"足不出户"的核心抱怨；占比温和下降
   （12.1%→10.2%，5/6）；**且没有恶化幼体渴死瓶颈**（`death_thirst_age` 反而 +4.9，§3.2 尾部标注的
   头号风险未兑现）。6 种子均未灭绝（carn% 地板 7.8%）。
3. **但"离开河道"只弱成立**（`carn_water_dist` 仅 +1.4、噪声大）。这坐实了 §3.2 的结构判断与
   `carnivore_riparian.md` 的核心洞见：**本世界水是硬约束、捕食者又恰恰驻扎在水边，"水边即最恐惧处"
   的公共信息无法在不把猎物推向渴死的前提下把捕食者赶离水**——真正缺的是**昼夜通勤**（猎物快饮即走），
   而这个无昼夜时钟的世界给不了。恐惧场能让捕食者"动起来、少一点"，给不了"搬离河岸"。

**落地决定**：把 `fear_rate=0.05`/`fear_sense_scale=3.0`/`fear_decay=0.99` 设为**默认开启**（用户明确
要求改动实际世界的行为），golden 已按预期重新 bless（200 步 smoke 里恐惧场几乎不累积，golden 仅微动：
population 1520→1549 等）。`--set fear_rate=0` 是消融对照 / 恢复旧行为。这是**可逆的**：不满意一行改回。

**未做 / 后续**：
- **"河岸带停留时长/到访频率"指标未加**（§3.2 可证伪预测第 3 条要求的 Courbin 2019 类比目标）——静态
  快照的距离均值分不清"缩短停留"和"完全不去"，当前只有 `carn_water_dist` 均值。若要判定机制究竟改变了
  哪种行为，需补这个指标。
- **真正的"捕食者搬离河岸"需要昼夜通勤或领地排斥**，恐惧场是必要不充分。见 `carnivore_riparian.md` §2.2b
  的领地机制与本文档设计三（局部密度代谢附加成本，前置条件是先有改善内陆猎物梯度的机制）。
- 设计一（危险记忆槽）、设计三仍未实现。
