...
## 8. 重设计的演化验证（2026-07-24，run_id: 20260724-spike-redesign）

- **假设**：进攻/防御双用 + venom 的重设计**盘活了 spike**——至少食肉进攻侧强演化；食草防御侧
  （venom 报复）可能也被盘活（这是重设计要回答的核心开放问题）。
- **成功判据**（只切 spike，隔离其效应）：
  - **Q1（进攻侧）**：`carn_spike` ON >> OFF，6/6 同向，配对 Wilcoxon p≤0.05——食肉进攻尖刺演化。
    预期强阳性（进攻是直接选择）。
  - **Q2（防御侧，核心）**：`herb_spike` ON > OFF，6/6 同向，p≤0.05——**判「重设计有没有盘活食草
    防御」**。§7 早期信号（herb_spike 0.016–0.045）提示可能仍弱，本实验给判决。
  - **Q3（护栏）**：ON 臂 `carn_frac` 健康（不灭绝不爆炸）、`population` 稳定；`mean_venom`>0 确认
    机制在开火。
- **对照臂**：ON = 默认（`spike_heritable=True`，`armor_heritable=True`）；OFF =
  `--set spike_heritable=False`（**`armor_heritable=True` 两臂都开、保持不变**，隔离 spike）。
- **种子**：0–5（6 配对），各 20000 步。**git hash**: 04d18e7。
- **结果**（12 run 全部无 NaN、无灭绝；出处 `outputs/20260724-spike-redesign/{on,off}_seed{0..5}.log`，
  统计 `explorations/20260725-spike-redesign/analyze.py`；配对 Wilcoxon + 10000 次 bootstrap CI，
  全部算过的 p 都列、不 Bonferroni；n=6 地板 p=0.031）：

  | 指标 | ON | OFF | 配对差 | boot 95% CI | p | 同向 |
  | --- | --- | --- | --- | --- | --- | --- |
  | **carn_spike** | 0.202 | 0.052 | +0.151 | [+0.103,+0.201] | **0.0312** | **6/6** |
  | **herb_spike** | 0.023 | 0.044 | −0.020 | [−0.051,+0.013] | 0.4375 | 2/6 |
  | mean_venom | 0.050 | 0.000 | +0.050 | [+0.040,+0.060] | **0.0312** | 6/6 |
  | carnivore_frac | 0.189 | 0.144 | +0.045 | [−0.005,+0.095] | 0.3125 | 4/6 |
  | population | 1728 | 1792 | −64 | [−222,+121] | 0.4375 | 2/6 |
  | herb_armor | 0.161 | 0.180 | −0.019 | [−0.087,+0.054] | 0.4375 | 2/6 |
  | carn_armor | 0.078 | 0.138 | −0.060 | [−0.145,+0.019] | 0.4375 | 3/6 |
  | hunt_success | 0.723 | 0.776 | −0.053 | [−0.118,+0.029] | 0.2188 | 2/6 |

  **carn_spike 逐种子**：seed0 0.191/0.067、1 0.161/0.007、2 0.263/0.015、3 0.171/0.073、4 0.208/0.138、
  5 0.219/0.010（ON/OFF，**6/6** ON>OFF）。**herb_spike 逐种子**：seed0 +0.047、1 +0.013、2 −0.046、
  3 −0.061、4 −0.064、5 −0.011（**2/6** ON>OFF，OFF 臂 0.0005–0.087 乱跳=纯漂变，不是被选择抬升）。

- **结论**：**重设计半盘活了 spike——进攻侧活了，防御侧没活。**
  - **[本世界实测] Q1 进攻侧达成。** `carn_spike` ON 0.202 vs OFF 0.052，**6/6 同向**，配对 Wilcoxon
    **p=0.0312**、rank-biserial +1.0、CI 全正。相对旧设计（§6，ON 0.019 还被 OFF 0.033 倒挂）≈**10.6×**、
    相对本次 OFF ≈3.9×。**「食肉尖刺=穿透/加伤 armor」的进攻收益被清晰选择**——用户「只有尖刺能杀」
    的直觉（改成「从旧基线往上加伤的乘子」以避开 founder 崩溃后）成立。
  - **[本世界实测] Q2 防御侧未达成。** `herb_spike` ON 0.023 **不高于** OFF 0.044（反而略低），2/6 同向、
    p=0.4375，**贴在旧设计 ~0.027 的漂变带内**。重设计没有盘活食草防御性尖刺。
  - **[本世界实测] Q3 护栏健康。** ON 臂 `carn_frac` 0.189（min 0.064，不灭绝不爆炸）、population ~1728
    稳定；`mean_venom` ON 0.050 vs OFF 0（6/6，p=0.0312）——**中毒机制确实在给攻击者下毒、在开火**。
  - **[对应] 防御侧为何仍不演化（诊断，与 §6 旧结论同根）**：`mean_venom`>0 证明毒**命中了攻击者**
    （不是机制没触发），但**毒发时被咬的猎物多半已经死了**——减速+耗能的惩罚落在攻击者未来几步，收益
    回退到亲缘/群体，被毒的个体自己拿不到边际存活收益，故防御性尖刺不被个体选择、绝对水平贴近 0。
    这正是 §7 预判的「时机陷阱」：debuff 没能降低「同一次致命咬击」的成功率。
  - **[提案，非结论] 救防御侧的方向**：让 venom 收益个体化——在**攻击瞬间**即给被咬猎物一个非零逃脱
    概率（收益回到猎物本人，而非攻击者未来几步），或大幅提高 `venom_slow`/`venom_drain` 让攻击者当次
    放弃追击；任一改动都要重跑 6 配对 ON/OFF 才能归因。carn_spike 的 p=0.031 在地板上，要更强证据补
    seed 6–11 到 12 配对。
  - **伪重复边界**：6 种子同一张地图；trait 均值非空间指标，`carn_frac`/`death_predation_frac` 绝对水平
    只推广到这一河系。分析脚本 `explorations/20260725-spike-redesign/`（入库，可复算）。
