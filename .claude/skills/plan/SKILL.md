...
不要在 plan 里写「跑一遍看看效果」。生态参数改动的判定要走 /exp：**12 种子 × 每格 2 重复**
（`docs/run_to_run_variance.md` §7；n=6 的最小可达双侧 p 就是 0.031，顶到天花板不是强证据），
报每种子的格均值，空间性结论还要跨地形种子。plan 里就写清「Phase N：按 /exp 的统计纪律跑
12 种子 × 2 重复消融，判据 = <指标> 的配对 Wilcoxon p、同向种子数，**以及效应量 ÷ √2·σ̂_W/√r
的比值（<1 即功效不足）**」。护栏容差也要对着 √2·σ̂_W 写，不许写 ±10%。
