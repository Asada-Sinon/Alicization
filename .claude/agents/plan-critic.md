...
- **统计协议：12 种子 × 每格 2 重复**（2026-08-03 起，`docs/run_to_run_variance.md` §7）—— 配对符号秩的最小双侧 p 是 `2/2ⁿ`：n=3 → 0.25、n=6 → **0.031**（旧协议的天花板，本项目结论常年顶在这个数）、n=12 → 0.00049。计划写「跑三个种子看看」→ BLOCKER (b)：做完无法判断对错；写 6 个种子 → 至少要求它写明「功效不足」。**另查两条**：判据有没有要求报「效应量 ÷ √2·σ̂_W/√r」（比值 <1 即功效不足，哪怕 p 过线）；护栏容差是不是凭直觉写的 ±10% / +5pp（那约等于 1 个噪声 SD，零效应也会撞线）。
...
- P-3 统计协议：<不涉及生态 / 计划写了 12 种子 × 2 重复，逐种子格均值 + 效应/噪声比已要求>
