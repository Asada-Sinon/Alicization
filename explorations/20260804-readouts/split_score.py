...
def split_score(n, ctr, smooth=1):
    ...
    **第一版是坏的，教训记在这里**：它对每个分割点 `t` 算
    ...
    if smooth:
        ...
        p = p / max(p.sum(), 1e-12)
    # 局部极大：**把整段平台当成一个峰**（端点用单侧）。
    #
    # **第四次同型失误，修在这里。** 初版用的是**严格**不等号：
    #     p[i] > p[i-1] and p[i] > p[i+1]
    # 于是一个被压缩到两个相邻箱、平滑后成为**平台**的簇（实测 `p[17]=p[18]=0.1647`）
    # **一个局部极大都找不到**，函数按构造返回 0。R17 实测：主臂 24 个 run 里
    # **12 个**这样返回 0，而对照臂 0 个——于是「分裂变糙了」这个假信号
    # （−0.2400、1/12、p=0.00098）整个是读数造的。**它恰恰在分裂变尖时失效**：
    # 簇越紧、越容易挤进相邻箱、越容易形成平台。
    #
    # **不能简单放宽成「不小于」**——那会让平台上每个点都算局部极大，
    # 于是 `len(loc) < 2` 的守卫失效、宽单峰会被读成双峰。
    # 正确做法是先合并等值的相邻段，再对整段判是否高于两侧，取段中点。
    # 严格单点峰是长度为 1 的平台，所以这是旧行为的**严格推广**。
    loc, i, m = [], 0, len(p)
    while i < m:
        j = i
