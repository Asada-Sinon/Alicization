...
- **PENDING（下次第一件事，可执行）**:
  1. `bash explorations/20260803-shade/sweep_r9.sh`（**断点续跑，重跑即补齐**）。
     **但先 `nvidia-smi` 看卡上有没有用户自己的任务。**
  2. 96 个 log 齐了 → `XLA_PYTHON_CLIENT_PREALLOCATE=false .venv/bin/python
     explorations/20260803-shade/analyze_r9.py` → **派 `result-analyst` 出判决**。
     主口径是**交互项** `d_s = (T1−N1) − (T0−N0)`，噪声 `2·σ̂_W/√r`（不是 `pair_noise`）。
