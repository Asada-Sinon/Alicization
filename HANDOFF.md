...
- 完成（可视化交付，用户点名要的）: Artifact
  **https://claude.ai/code/artifact/285960f6-abfd-465f-8d82-e50cdc61bf6d**（默认私有）——
  ...
  - **窄窗口补测在跑**（`L_tradeoff0125` / `N_tradeoff025`，36 run，同目录）——
    这是 §9.10 限制 2 的收口，纯 `--set`、不改世界、不需要拍板。
    **看结果：若两点的 `sd` 都不高于中性对照 0.09508，则「无分化窗口」收口。**
- **PENDING（下个 session 第一件事）**:
  1. 看窄窗口补测结果（若已完成则本轮全部收口）。
  2. **把下面三件拿给用户拍板，不要自己挑一个开跑。**
  六个回合全部走完、全部落档，`docs/multispecies_program.md §4` 的 backlog 已按判决重排完毕。
